...
`
    function makeSimulation(graph, aspect) {
      ...
      const nodes = graph.nodes.map((node) => {
        const degree = (graph.childrenOf.get(node.id) || []).length + (graph.consumes.get(node.id) || []).length + (graph.provides.get(node.id) || []).length
        ...
        return { id: node.id, x: 0, y: 0, vx: 0, vy: 0, fixed: false, r: node.isGroup ? 8 : 5 + Math.min(7, Math.log(1 + degree) * 1.6), color: colors.color, lit: colors.lit, ring: colors.ring, label, data: node }
      })
